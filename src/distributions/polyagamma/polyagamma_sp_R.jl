...
function rrtigauss_1(rng::Distributions.AbstractRNG, mu::Float64, lambda::Float64, trnc::Float64)
...
function rltgamma_dagpunar_1(rng::Distributions.AbstractRNG, shape::Float64, rate::Float64, trnc::Float64)
...
function pigauss(x::Float64, Z::Float64, lambda::Float64)
  ...
  y = Distributions.cdf(Distributions.Normal(),b) + exp(2 * lambda * Z) * Distributions.cdf(Distributions.Normal(),a);
                                        # y2 = 2 * pnorm(-1.0 / sqrt(x));
  return y
end
##------------------------------------------------------------------------------
## Generate sample for J^*(n,z) using SP approx.
function sp_sampler_1(rng::Distributions.AbstractRNG,n::Float64, z::Float64; maxiter=1000)
  ...
  wl = al^0.5 * _exp_c(-n*sqrt(2*rl) + n * il + 0.5*n - 0.5*n*(1-1/mid)) * pigauss(mid, 1/sqrt(2*rl), n)
  #Distributions.cdf(Distributions.InverseGaussian(sqrt(2*rl), n), mid)  
  wr = ar^0.5 * (0.5*n/pi)^0.5 * _exp_c(-n * _log_c(n * rr) + n * ir - n * _log_c(mid) + loggamma(n))* (1-Distributions.cdf(Distributions.Gamma(n, 1/n*rr), mid))
  ...
  while (go && iter<maxiter) 
    ...
    if (wt*Random.rand(rng) < wl) 
      ...
      FX = al^0.5 * (0.5 * n / pi)^0.5 * X^(-1.5) * _exp_c(phi_ev);
