...
minimum(d::Distributions.Truncated{Distributions.InverseGaussian{T},Distributions.Continuous}) where {T<:Real} = d.lower
maximum(d::Distributions.Truncated{Distributions.InverseGaussian{T},Distributions.Continuous}) where {T<:Real} = d.upper

## sampling

### sampler for upper truncated inverse gaussian

function Distributions.rand(rng::Distributions.AbstractRNG, d::Distributions.Truncated{Distributions.InverseGaussian{T},Distributions.Continuous}) where {T<:Real}
    μ = d.untruncated.μ
    λ = d.untruncated.λ
    if d.lower == 0.0
        X = d.upper + 1;
