...
items = [Item2PL(i, string("item_", i), ["math"], Parameters2PL(Product([LogNormal(0.0,0.25), Normal(0,1)]), [1e-5,Inf], [-Inf, Inf])) for i = 1:I];
...
items_est = [Item2PL(i, string("item_",i), ["math"], Parameters2PL(Product([TruncatedNormal(1, 5, 0.0, Inf), Normal(0, 5)]), [1e-5,5.0], [-6.0, 6.0])) for i = 1 : I];
...
items_idx_per_examinee = map( e -> sort(map( r -> items_est[r.item_idx].idx, responses_per_examinee[e.idx])), examinees_est);

responses_per_item = map( i -> sort(get_responses_by_item_id(i.id, responses), by= x-> x.examinee_idx), items);
examinees_idx_per_item = map( i -> sort(map( r -> examinees_est[r.examinee_idx].idx, responses_per_item[i.idx])), items_est);
...
for iter = 1:Iter
   ...
   W = generate_w(items_est, map(i -> examinees_est[examinees_idx_per_item[i.idx]], items_est))
#    map( i -> begin
...
#    end, items_est)
   map( i -> mcmc_iter!(i, examinees_est[examinees_idx_per_item[i.idx]], responses_per_item[i.idx], map( y -> y.val, sort(filter(w -> w.i_idx == i.idx, W), by= x -> x.e_idx)); sampling = false), items_est)
   map( e -> mcmc_iter!(e, items_est[items_idx_per_examinee[e.idx]], responses_per_examinee[e.idx], map( y -> y.val, sort(filter(w -> w.e_idx == e.idx, W), by= x -> x.i_idx)); sampling = false), examinees_est)
